...
// The live trading terminal: every tunable exit/size knob the trader is running
// RIGHT NOW, adjustable in real time. Two authorities feed each dial — the
// adaptive policy (data-derived, shown as a ghost recommendation) and the
// operator's manual pin (always wins). The regime read up top is what the policy
// is reacting to. Auto ships in ADVISORY: computed and shown, not applied, until
// a clean prime run gives the policy its favorable-regime pole.
...
export function ControlTerminal({ view }: { view: ControlTerminalView }) {
  ...
  const pinned = view.groups.flatMap((g) => g.knobs).filter((k) => k.manual != null).length;

  return (
    <div className="w-full">
      {/* regime read + auto-mode authority */}
      <div className="mb-3 rounded-md p-3" style={{ background: "var(--surface-1)", border: "1px solid var(--border)" }}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold" style={{ color: hostTone }}>
                {r ? r.label : "warming up…"}
              </span>
            </div>
            {r && (
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] tabular" style={{ color: "var(--text-secondary)" }}>
        ...
        {view.autoMode === "live" && (
          <div className="mt-2 text-[11px]" style={{ color: "var(--status-warning)" }}>
            ⚠ Live — the policy is driving unpinned knobs. It is calibrated almost entirely on hostile farm tape;
            its favorable-regime side is a prior until a clean prime run validates it.
          </div>
        )}
        {view.autoMode === "advisory" && (
          <div className="mt-2 text-[11px]" style={{ color: "var(--text-muted)" }}>
            Advisory — policy shown as the <span style={{ color: "var(--series-1)" }}>ghost</span> value; the trader runs default/pinned only.
          </div>
        )}
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-[11px]" style={{ color: "var(--text-muted)" }}>
        <span>
          ...
          {" · "}off-hours size ×{view.effectiveNote.offHoursMult}
          {view.effectiveNote.primeNow ? " · PRIME now" : " · off-hours now"}
        </span>
        {pinned > 0 && (
          <button
