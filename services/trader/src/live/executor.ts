...
export function liveLaneStatus(cfg: HermesConfig): string {
  ...
  const selection = cfg.LIVE_MIRROR_PAPER
    ? `, 🪞 MIRROR paper entries on executable venues [${cfg.LIVE_MIRROR_VENUES}] (∪ smart-money rescue ≥${cfg.LIVE_WALLET_RESCUE_MIN_WINNERS} winner-wallets); honeypot trap-only`
    : `${cfg.LIVE_PREMIUM_ONLY ? `, PREMIUM venues (∪ smart-money rescue ≥${cfg.LIVE_WALLET_RESCUE_MIN_WINNERS} winner-wallets)` : ""}`;
  return `live lane: ARMED — wallet ${w.publicKey.toBase58()}, SIZER ${(cfg.LIVE_SIZE_FRAC * 100).toFixed(0)}% of balance (min $${cfg.LIVE_MIN_POSITION_USD}, ≤${(cfg.LIVE_MAX_POSITION_FRAC * 100).toFixed(0)}%/pos), exposure ≤${(cfg.LIVE_MAX_EXPOSURE_FRAC * 100).toFixed(0)}%, daily −$${cfg.LIVE_DAILY_LOSS_CAP_USD}, kill −$${cfg.LIVE_KILL_LOSS_USD}${selection}${cfg.LIVE_WALLET_GATE ? " + wallet-graph rug gate" : ""}${cfg.LIVE_REGIME_GATE ? ` + regime gate (${cfg.LIVE_MIRROR_PAPER ? `mirror-venues edge ≤−${(cfg.LIVE_MIRROR_REGIME_MAX_LOSS_PCT * 100).toFixed(0)}%` : `paper ≤−$${cfg.LIVE_REGIME_MAX_LOSS_USD}`}/${cfg.LIVE_REGIME_WINDOW_MIN}m stands down)` : ""}${cfg.LIVE_GUARD_ENABLED ? ` + fast protective stop (mark-priced, cut at −${cfg.LIVE_STOP_PCT}% / dump @ ${(cfg.LIVE_STOP_SLIPPAGE_BPS / 100).toFixed(0)}%)` : ""}${cfg.CONVICTION_ENABLED ? ` + conviction sizing (${cfg.CONVICTION_SIZE_MIN_BAND}–${cfg.CONVICTION_SIZE_MAX_BAND}× band)` : ""}${cfg.LIVE_ANTICIPATION_ENABLED ? ` + anticipation tilt (venue-momentum × tail-odds, ${cfg.LIVE_ANTICIPATION_MIN}–${cfg.LIVE_ANTICIPATION_MAX}×)` : ""}${cfg.LIVE_ANTICIPATION_GATE ? ` + anticipation GATE (stand down when cold, <${cfg.LIVE_ANTICIPATION_GATE_MIN}×)` : ""}`;
}
