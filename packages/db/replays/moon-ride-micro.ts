...
const HAIRCUT = 0.98;
const MILESTONES = [1.5, 2, 2.5, 3, 5, 8, 13, 21, 34, 55];
const RATCHET = 0.7;
...
for (const p of positions) {
  ...
  if (maxMult < 1.22) {
    ...
    let peak = 0;
    let floor = 0;
    let exited = false; const sold = new Set<number>();
    for (const t of path2) {
      ...
      for (const ms of MILESTONES) if (peak >= ms) { if (ms >= 2.5 && !sold.has(ms) && held > 0.05) { cash += fill(ms, 0.05, t.liq); held -= 0.05; sold.add(ms); } floor = Math.max(floor, ms * RATCHET); }
