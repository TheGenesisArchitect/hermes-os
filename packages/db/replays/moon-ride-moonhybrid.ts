...
const positions = await sql`
  SELECT p.signature, p.id, p.mint, t.symbol, p.size_usd::float AS size, p.realized_pnl_usd::float AS pnl,
...
for (const p of positions) {
  ...
  if (maxMult < 1.22) {
    ...
    for (const t of path2) {
      ...
      const isMoon = typeof p.signature === "string" && p.signature.startsWith("MOON"); const glFloor = isMoon && peak < 1.5 && ri > 0 ? 1 + (peak - 1) * 0.65 : 0; const effFloor = Math.max(floor, glFloor); if (effFloor > 0 && t.m <= effFloor) {
        cash += fill(effFloor, held, t.liq);
        held = 0;
        exited = true;
        break;
      }
  ...
  const key = (typeof p.signature === "string" && p.signature.startsWith("MOON")) ? "MOON" : (p.signature ?? "unrouted");
